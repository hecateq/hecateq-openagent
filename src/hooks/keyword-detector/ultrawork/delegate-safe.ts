/**
 * Delegate-safe ultrawork message for Hecateq orchestrator.
 *
 * This variant strips self-execution directives ("IMPLEMENT EXACTLY",
 * "DO IT YOURSELF", "DELIVER EXACTLY X", "ZERO TOLERANCE FAILURES")
 * while preserving the verification, quality assurance, certainty
 * protocol, and delegation-first sections that remain valuable for
 * the orchestrator.
 *
 * Used when:
 *   currentAgent === "hecateq-orchestrator"
 *   AND hecateq.orchestrator.delegation_first !== false (default true)
 *
 * Without this gate, the default ultrawork message injects contradictory
 * self-execution language into the Hecateq orchestrator that directly
 * conflicts with its delegation-first policy.
 */
export const ULTRAWORK_DELEGATE_SAFE_MESSAGE = `<ultrawork-mode>
...
## DELEGATION-SAFE NOTE

This is a delegation-safe ultrawork mode for the Hecateq orchestrator.
You are an orchestrator. Your job is to understand, plan, delegate, and verify.
Implementation details belong to the specialist agents you dispatch.
Focus on quality of delegation over quality of direct implementation.

</ultrawork-mode>

`

export function getDelegateSafeUltraworkMessage(): string {
  return ULTRAWORK_DELEGATE_SAFE_MESSAGE
}

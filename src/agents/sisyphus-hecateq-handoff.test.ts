...
describe("Sisyphus Hecateq handoff policy", () => {
  test("injects prompt-level Hecateq handoff guidance into Sisyphus without changing Hecateq-only policy blocks", async () => {
    ...
    try {
      ...
      expect(agents["hecateq-orchestrator"].prompt).toContain("AGENT INDEX POLICY")
      expect(agents["hecateq-orchestrator"].prompt).toContain("The generated agent index is a ranking and selection aid, not runtime truth.")
      ...
      expect(agents.sisyphus.prompt).not.toContain("AGENT INDEX POLICY")

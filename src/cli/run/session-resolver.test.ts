...
describe("resolveSession", () => {
  ...
  it("creates new session when no session ID provided", async () => {
    ...
    expect(mockClient.session.create).toHaveBeenCalledWith({
      body: {
        title: "@hecateq/hecateq-openagent run",
        permission: [
  ...
  it("retries session creation on failure", async () => {
    ...
    // when
    const result = await resolveSession({ client: mockClient, directory, sleep: async () => {} })
  ...
  it("throws after all retries exhausted", async () => {
    ...
    // when
    const result = resolveSession({ client: mockClient, directory, sleep: async () => {} })
